...
func _mouse_left_click() -> void:
	mouse_click_space.delete_list(self)
...
func _add_new_collison_space () ->void:
	...
	var LeftPointOne = MouseClickSpace.PolygonPoint.new(Vector2i(CenterPoint.x - (size.x/2),0))
	LeftPointOne.next_point = MouseClickSpace.PolygonPoint.new(Vector2i(CenterPoint.x - (size.x/2),CenterPoint.y + (size.y/2)))
	LeftPointOne.next_point.last_point = LeftPointOne
	LeftPointOne.next_point.next_point = MouseClickSpace.PolygonPoint.new(Vector2i(CenterPoint.x + (size.x/2),CenterPoint.y + (size.y/2)))
	LeftPointOne.next_point.next_point.last_point = LeftPointOne.next_point.next_point
	LeftPointOne.next_point.next_point.next_point = MouseClickSpace.PolygonPoint.new(Vector2i(CenterPoint.x + (size.x/2),0))
	...
	mouse_click_space.insert_list(LeftPointOne,LeftPointOne.next_point.next_point.next_point,self,MouseClickSpace.PointType.TOP_LEFT,MouseClickSpace.PointType.TOP_RIGHT)
	mouse_click_space.redraw_mouse_passthrough_polygon()
