extends Path2D
class_name SpecialShapedWindow
...
var _bottom_size:float=300  #底部边框宽度
var _offset:float=0 #底部边框高度偏移

var _bottom_height_change:bool = true
#endregion
enum PointType
{
	TOP_LEFT = 0,
...
func _init_mouse_click_space(minsize:float = 1)->void:
	_screen_size = DisplayServer.window_get_size()#初始化屏幕大小！！！！！！！！！！！！！！！如果动态调整屏幕分辨率后导致异形窗口绘制出错，需要在这里修改
	DisplayServer.window_get_size()
	_minsize = minsize
	#_screen_size = _screen_size + Vector2i(0,0)
	#初始化anchorPoint
	anchor_head_point = PolygonPoint.new(Vector2i(0,_screen_size.y),PointType.A_BOTTOM_LEFT)                              #左下角
	anchor_head_point.next_point = PolygonPoint.new(_screen_size,PointType.A_BOTTOM_RIGHT)                                 #右下角
	
	anchor_head_point.next_point.last_point = anchor_head_point
	anchor_head_point.next_point.next_point = PolygonPoint.new(Vector2i(_screen_size.x,0),PointType.A_TOP_RIGHT)        #右上
	
	anchor_head_point.next_point.next_point.last_point = anchor_head_point.next_point
	anchor_tail_point = PolygonPoint.new(Vector2i(0,0),PointType.A_TOP_LEFT)                                           #左上
...
func _ready() -> void:
	#初始化完毕，绘制异形窗口
	AirshipWindowController.message_bottom_height_change.connect(_on_window_bottom_offset_change)
	redraw_mouse_passthrough_polygon()
	set_bottom_size(_bottom_size)
	pass
...
func _reload_all_point () -> void:
	...
	while point != null:
		if _bottom_height_change:
			match point.type:
				PointType.A_BOTTOM_LEFT:
					point.pp_position.y = point.pp_position.y - _offset
				PointType.A_BOTTOM_RIGHT:
					point.pp_position.y = point.pp_position.y - _offset
	...
	_bottom_height_change = false

#region 回调函数

func _on_window_bottom_offset_change(height_offset:float)->void:
	_bottom_height_change = true
	_offset = height_offset
	_reload_all_point()
	redraw_mouse_passthrough_polygon()
	pass
#endregion
...
func set_bottom_size(new_bottom_size)->void:
	bottom_left_point.pp_position = Vector2(bottom_left_point.pp_position.x,anchor_head_point.pp_position.y - new_bottom_size)
	bottom_right_point.pp_position = Vector2(bottom_right_point.pp_position.x,anchor_head_point.next_point.pp_position.y - new_bottom_size)
	_reload_all_point()
	redraw_mouse_passthrough_polygon()
...
func set_bottom_volum_size_and_offset(size:float = 0,offset:float = 0)->void:
	_bottom_size = size
	_offset = offset
